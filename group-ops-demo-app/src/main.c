...
io_seproxyhal_touch_approve(const bagl_element_t *e) {
  ...
  // const group *pp = &p;
  // group w = affine_scalar_mul(bigk, &p);
  ...
  */
  ...
  // group_scalar_mul(&projq, three, &p);
  // group_double(&qq, &p);
  // group_double(&projq, &p);
  // group_add(&projq, &qq, &p);
  group_double(&qq, &p);
  projective_to_affine(&aq, &qq);
  os_memmove(G_io_apdu_buffer, aq.x, field_BYTES);
  tx = field_BYTES;

  // os_memmove(G_io_apdu_buffer + tx, aq.x, field_BYTES);
  // tx += field_BYTES;
