...
static const bagl_element_t *io_seproxyhal_touch_approve(const bagl_element_t *e) {
  unsigned int tx = 0;
  unsigned char cxy[2*fmnt6753_BYTES];
  /*
  bool ioc_a = is_on_curve(a);
  G_io_apdu_buffer[tx++] = ioc_a;
  bool ioc_b = is_on_curve(b);
  G_io_apdu_buffer[tx++] = ioc_b;
  */
  ...
  gmnt6753 *c;
  gmnt6753_affine_add(c, a, b);
  os_memmove(cxy, c->X, fmnt6753_BYTES);
  os_memmove(cxy + fmnt6753_BYTES, c->Y, fmnt6753_BYTES);
  os_memmove(G_io_apdu_buffer, cxy, 2*fmnt6753_BYTES);
  tx = 2*fmnt6753_BYTES;
