...
const scalar round_keys[ROUNDS][SPONGE_SIZE] = {
    {
...
const scalar MDS[SPONGE_SIZE][SPONGE_SIZE] = {
    {
...
void alphath_root(scalar x) { scalar_pow(x, x, alpha_inv); }

void to_the_alpha(scalar x) { scalar_pow(x, x, alpha); }
...
void poseidon(scalar input) {
  ...
  for (int r = 0; r < half_rounds; r++) {
    for (int i = 0; i < SPONGE_SIZE; i++) {
      scalar_add(state[i], input, round_keys[r][i]);
      to_the_alpha(state[i]);
      scalar_mul(state[i], state[i], MDS[r][i]);
    }
  }
  int k = half_rounds;
  // all partial rounds
