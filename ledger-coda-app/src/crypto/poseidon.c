...
const field alpha = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
const field round_keys[ROUNDS][SPONGE_SIZE] = {
    {
...
const field MDS[SPONGE_SIZE][SPONGE_SIZE] = {
    {
...
void alphath_root(field c, field x) {
  ...
  cx_math_powm(c, x, alpha_inv, field_BYTES, field_modulus, field_BYTES);
}
...
void poseidon(field state, const field input) {
  int full_rounds = 8;
  int half_rounds = 4;
  ...
  for (int r = 0; r < half_rounds; r++) {
    for (int i = 0; i < SPONGE_SIZE; i++) {
        field_add(state, input, round_keys[r][i]);
        to_the_alpha(state[r][i]);
        field_mul(state, state, MDS[r][i]);
    }
  }
  int k = half_rounds;
  // all partial rounds
